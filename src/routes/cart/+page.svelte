	...
	let items: CartItem[] = $state([
		{
			...
			variant:
				data.products.find((p) => p.id === 'oyster-001')?.type === 'withShell' ? 'kg' : 'パック',
			...
			type: (data.products.find((p) => p.id === 'oyster-001')?.type ||
				'noImage') as CartItem['type']
		},
		{
			id: 'oyster-005',
			title: data.products.find((p) => p.id === 'oyster-005')?.name || '不明な商品',
			variant:
				data.products.find((p) => p.id === 'oyster-005')?.type === 'withShell' ? 'kg' : 'パック',
		...
		},
		{
			...
			variant:
				data.products.find((p) => p.id === 'oyster-003')?.type === 'withShell' ? 'kg' : 'パック',
